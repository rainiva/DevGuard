...
- Use when: any non-trivial implementation, refactor, migration, or performance-sensitive change after required project understanding and required official docs check.
- Risk tags: `api_contract`, `data_consistency`, `migration`, `release`, `security`, `performance`, `compatibility`, `auth_permission`

## Summary

Understand what the change touches before editing code.

Impact analysis is the change-impact judgment pass. It is not project understanding, official docs check, or the Task Contract. Default the outward report to the smallest shape that still preserves safe execution; keep `Impact Analysis Summary` internal unless scope, blockers, or reroute require outward disclosure.
...
Use `Impact Analysis Summary` from `references/report-templates.md` as the internal record shape.

Keep this internal by default for ordinary `FAST` and `STANDARD` work. Emit it outward only when the impact finding changes scope, blocks execution, requires reroute, creates an anomaly, or cannot fit in `Risk Note` or `Exception Note`.
...
- In default outward output, do not emit a separate `Impact Analysis Summary` block unless the routed disclosure trigger requires it; keep the impact-analysis record internal.
